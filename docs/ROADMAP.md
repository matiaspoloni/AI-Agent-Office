...
| **10 – Packaging** | NSIS `AgentOfficeSetup.exe` (Start Menu, desktop shortcut, uninstaller), sidecar bundling, WebView2 bootstrapper, version metadata, optional signing hook, updater plumbing | Clean-VM install/uninstall smoke test |
...
| R7 | Hook latency slows the agent. | Worse agent UX. | Relay is a tiny native exe, async hooks for non-decision events, 200 ms connect timeout, exit 0 when the app is closed. |
